...
class Mock {
    ...
    fun getPhrases(view: Int): String {
        // Get phrase
        val filter = listPhrases.filter { it.categoryId == view || view == all }
        // Generate random number
        val random = Random.nextInt(filter.size)
        // Return phrase
        return filter[random].description
    }
